...
// Cập nhật tài khoản vào ngăn xếp
...
// Check ID
const ref_checkID = ref(db, "Current");
onValue(ref_checkID, (snapshot) => {
  ...
  if (current.id != null) {
    ...
    for (var i = 0; i < accountStack.length; i++) {
      ...
      if (current.status == 1) {
        // Kiểm tra id so với id trong ngăn xếp
        if (current.id == account.id && account.status == "Chưa thuê") {
          update(ref(db, "Current"), { id: "" });
          update(ref(db, "Current"), { checkID: 1 });
        }
      } else if (current.status == 2) {
        // Kiểm tra id so với id trong ngăn xếp
        if (current.id == account.id && account.status == "Đã thuê") {
          update(ref(db, "Current"), { id: "" });
          update(ref(db, "Current"), { checkID: 1 });
        }
      }
